...
import com.grupo52.tech_challenge.gateway.impl.SendNotaFiscalEmailGatewayImpl;
...
class SendNotaFiscalEmailGatewayImplTest {

    @Mock
    private JavaMailSender mailSender;

    @Mock
    private NotaFiscalDocumentGateway notaFiscalDocumentGateway;

    @InjectMocks
    private SendNotaFiscalEmailGatewayImpl gateway;
    ...
    void executeSucesso() throws Exception {
        ...
        when(notaFiscalDocumentGateway.buildHtml(os)).thenReturn("<html>Nota Fiscal</html>");
        when(notaFiscalDocumentGateway.buildPdf(os)).thenReturn(new byte[]{1, 2, 3});
    ...
    void executeClienteSemEmailLancaGatewayException() {
        ...
        verifyNoInteractions(mailSender, notaFiscalDocumentGateway);
    }
    ...
    void executeErroAoConstruirPdfPropagaGatewayException() throws Exception {
        ...
        when(mailSender.createMimeMessage()).thenReturn(realMimeMessage());
        when(notaFiscalDocumentGateway.buildHtml(os)).thenReturn("<html>Nota Fiscal</html>");
        when(notaFiscalDocumentGateway.buildPdf(os)).thenThrow(new GatewayException("Falha ao gerar PDF"));
